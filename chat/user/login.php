...
if (!empty($_POST['btnLogin'])) {
    ...
    if (empty($username)) {
        $username_error_message = 'Le champ "Email / Nom d\'utilisateur" est requis !';
        array_push($login_error_message, $username_error_message);
    }
    if (empty($password)) {
        $password_error_message = 'Le champ "Mot de passe" est requis !';
        array_push($login_error_message, $password_error_message);
    }
    if(!empty($username) && !empty($password)) {
        ...
        if($user_id > 0) {
            ...
            $precedingURL = $session->has('curURL');
            if ($precedingURL) {
                header('Location: ' . $session->get('curURL'));
            } else {
        ...
        {
            $login_invalid = 'Informations de connexion invalide!';
            array_push($login_error_message, $login_invalid);
        }
    }
}
...
<head lang="FR-fr">
    <meta charset="utf-8" />
    <meta name="description" content="Page de connexion pour les membres enregistrés" />
...
<form action="login.php" method="post" id="userLoginForm">
    ...
    <?php
        if (!empty($login_error_message)) {
            foreach($login_error_message as $message) {
    ...
    <div class="inputGroup inputGroup1">
        <label for="email1">Email / Nom d'utilisateur</label>
        <input type="text" id="email" class="email" maxlength="256" name="username"/>
        <p class="helper helper1">email@domain.com / John Doe</p>
    ...
    <div class="inputGroup inputGroup3">
        <button id="login" type="submit" name="btnLogin"  form="userLoginForm" value="Submit">S'identifier</button>
    </div>
    <div class="inputGroup inputGroup4">
        <button type="button" class="registerLocation" name="btnRegister" onclick="window.location.href='registration.php'">Créer un compte</button>
    </div>
</form>
<footer>
